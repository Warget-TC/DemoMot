...
class DbConnexion
{
    ...
    public function addUser($pseudo, $password, $fristName, $lastName, $email, $name_file, $description)
    {
        //
        $this->dbConnect();

        // Ajoute les élements à ajouter
        $sql = "INSERT INTO t_member(idMember, memPseudo, memPassword,memFirstName, memLastName, memMail, memAvatar,memDescription)VALUES('','$pseudo','$password','$fristName','$lastName','$email', '$name_file', '$description')";// bdd =private bdd prepare $callteacher à être exectuer
        $rs = $this->bdd->prepare($sql);
        $rs->execute();
...
//VALUES ('','$nom','$code','$explication','$syntaxe','','','','',3);)";// bdd =private bdd prepare $callteacher à être exectuer
    ...
    public function addComment ($comment,$userPseudo){
        $this ->dbConnect();
        // Requête correct INSERT INTO t_comment (fkMember,fkScript,comComment)VALUES (10,18,'sfdsfdfsfsdfsdfsf')

        $sql = "INSERT INTO t_comment (idComment,comPseudo,comComment,fkScript)
        VALUES ('','$userPseudo','$comment',". $_POST['id'] . ");)";
        $rs = $this->bdd->prepare($sql);
        $rs->execute();
    ...
    public function getComment($idScript)
    {
        $this->dbConnect();

        $callComment = "select * from t_comment where fkScript = " . $idScript;

        $rs = $this->bdd->prepare($callComment);
        $rs->execute();
